...
def get_hair_care_advice(hair_type, porosity):
...
def chat_with_hair_expert(messages):
    system_message = {
        "role": "system",
        "content": """
        You are a professional hair care expert. 
...
def perform_ocr_and_analyze(image, hair_type, porosity):
    image = np.array(Image.open(image).convert("RGB"))[:, :, ::-1]
    gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    thresh = cv.threshold(gray, 0, 255, cv.THRESH_BINARY | cv.THRESH_OTSU)[1]
    extracted_text = pytesseract.image_to_string(thresh)

    prompt = f"""As a professional hair care expert, provide a detailed analysis of the following text extracted from a hair product label for {hair_type} hair with {porosity} porosity:
